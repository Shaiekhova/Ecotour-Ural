...
function applyFilters() {
  const filteredTours = allTours.filter((tour) => {
    ...
    // Проверка по фильтру из #all-toursFilter
    const grandSeasonMatch =
      selectedGrandFilters.season.length === 0 ||
      selectedGrandFilters.season.some((val) => tourSeasons.includes(val));
    const grandDurationMatch =
...
function displayTours(tours) {
  ...
  const isPortrait = window.innerWidth < window.innerHeight; // ориентация
  ...
  // Логика отображения при мобильной или вертикальной ориентации
  if (isMobileView || isPortrait) {
    ...
    // При ширине > 900 и горизонтальной ориентации
    // Очищаем оба ряда
    topRow.innerHTML = "";
...
// Обновление отображения при изменении окна или ориентации
function updateDisplayOnResize() {
  // Перезапускаем отображение с текущими фильтрами
  displayTours(applyCurrentFilters());
}

// Обработчики resize и orientationchange
window.addEventListener("resize", updateDisplayOnResize);
window.addEventListener("orientationchange", updateDisplayOnResize);

// Для этого создадим функцию, которая возвращает текущие отфильтрованные туры
